...
| Unsent composer text | Composer Draft State | Keep one in-memory text draft per Deep Code Task plus one separate new-task draft. Task switching saves the old scope and restores only the new scope; a successful send clears only its source. Draft text is not persisted or sent until the user submits it. |
...
| Human completion summary | Task Outcome + Recovery Projection | Project only terminal Engine state, confirmed changed files, explicit test/check/build commands, failed tools, and missing-verification warnings. A cancellation request without a matching terminal is a yellow pending state, not a stopped Turn; later terminal evidence replaces stale unconfirmed wording. Explain cause, impact, safety state, and the next human action. Never infer success from assistant prose; link every summary back to Run Details evidence. |
...
| Visual work receipt | Outcome Map Projection | Convert the completion summary and local path-only baseline assessment into a small versioned graph IR containing result, confirmed changes, verification, change attribution, open cautions, and next action. Harness remains execution truth; the local baseline only explains pre-existing workspace state. An optional future Archify Adapter may enrich the explanation but cannot replace either source. |
