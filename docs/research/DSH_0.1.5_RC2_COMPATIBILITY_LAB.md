...
- `DshAdapterV2.openTokenUsageControl()` requires the official `session/control` baseline to contain the exact Session and four non-negative integer `tokenUsage` buckets. Its remaining stream yields only strictly newer `tokenUsage` replacements for that Session; Host queues/jobs, other Sessions, unrelated projections, stale sequences, and unknown fields are discarded;
- Candidate Session Lab captures its newest cumulative control sample at Prompt preparation rather than reusing the older attach-time cut. After the bound Turn settles, the Prompt evidence calls usage `matched` only when all four cumulative deltas equal the same-Turn Assistant samples. A differing delta becomes `usage-mismatch`; absent, stale, or regressed control evidence remains incomplete;
...
- on 2026-09-15, one newly authorized 81-token revalidation printed the repaired Deep Code receipt in 6.6 seconds with durable admission, exact route, matching completed terminal, matched Assistant/Session-control usage, one request header, and zero retries. Gate D is accepted; another Provider call is neither needed nor authorized;
- `0.1.5-rc.2` remains product-disabled. Enabling it requires a separate product-activation review rather than extending this paid protocol check.
...
- After adding the control-stream baseline/delta seam, focused V2 Adapter/Prompt evidence/Lab/Supervisor behavior passed **43/43**, and full regression passed **363/363**. Source and packaged Supervisor smokes each opened two exact Session control streams (`usageControlsAttached: 2`) while retaining product admission closed and `providerRequests: 0`, `promptRequests: 0`.
- The current local test portable is `dist\Deep-code-Test-0.8.1-beta.1.exe`, 368,914,545 bytes, SHA-256 `3cb7af890690464d931b6178c23274b430a8e43b56a650867419375175c704e7`. Packaged/source hashes matched for Runtime Supervisor `0e1b2786...0ff5`, Candidate Session Lab `7b0e0f5a...d725`, Candidate Prompt Evidence `bc34b58e...f702`, DSH Adapter V2 `9345f75f...18fb`, and managed connection `bb22451a...09f4`.
