...
test('never creates a receipt from ordinary chat, a missing request header, or a mismatched route', () => {
  ...
  assert.equal(projectModelVerificationReceipt(dedicatedThread({ agent: { effectiveModel: { available: false }, runDetails: { terminal: { state: 'completed' } } } })), null)
  assert.equal(projectModelVerificationReceipt(dedicatedThread({ agent: { effectiveModel: { available: true, provider: 'deepseek', id: 'deepseek-v4-flash', reasoningEffort: 'low' }, runDetails: { terminal: { state: 'completed' } } } })), null)
  assert.equal(projectModelVerificationReceipt(dedicatedThread({ agent: { effectiveModel: { available: true, provider: 'zai-coding-cn', id: 'glm-4.5-air', reasoningEffort: 'low', evidence: 'request/header' }, runDetails: { terminal: { state: 'completed' } } } })), null)
})
