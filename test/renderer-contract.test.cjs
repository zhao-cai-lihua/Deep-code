...
test('model service manager separates configuration facts from real verification', () => {
  const viewScript = html.indexOf('src="./model-services-view.cjs"')
  const shellScript = html.indexOf('src="./shell.js"')
  assert.ok(viewScript >= 0 && viewScript < shellScript)
  ...
  assert.match(modelServicesView, /provider\[field\]/)
  assert.match(modelServicesView, /读取这些状态不会调用模型/)
  assert.match(shell, /modelServicesView\.render\(snapshot\)/)
...
test('a dedicated model test can show a persisted Harness-attributable receipt', () => {
  ...
  assert.match(taskEvidenceView, /模型验证回执/)
  assert.match(taskEvidenceView, /路线证据：Harness 请求头/)
...
test('connection verification binds an explicit Harness route and never trusts an empty session default', () => {
  assert.match(modelServicesView, /验证这个服务/)
...
test('terminal tasks expose one projected next-action panel with real controls', () => {
  const evidenceScript = html.indexOf('src="./task-evidence-view.cjs"')
  const shellScript = html.indexOf('src="./shell.js"')
  assert.ok(evidenceScript >= 0 && evidenceScript < shellScript)
  ...
  assert.match(shell, /onGuidanceAction: runGuidanceAction/)
  assert.match(taskEvidenceView, /onGuidanceAction/)
...
test('conversation separates the final answer from collapsible run evidence', () => {
  const messageViewScript = html.indexOf('src="./conversation-message-view.cjs"')
  const shellScript = html.indexOf('src="./shell.js"')
  assert.ok(messageViewScript >= 0 && messageViewScript < shellScript)
  ...
  assert.match(shell, /conversationMessageView\.render/)
  assert.match(conversationMessageView, /renderMarkdown/)
  assert.match(taskEvidenceView, /不作为你的发言显示/)
  ...
  assert.match(taskEvidenceView, /renderDiffCard/)
  assert.match(taskEvidenceView, /renderTerminalCard/)
  assert.match(taskEvidenceView, /renderReadCard/)
  assert.match(taskEvidenceView, /renderSearchCard/)
  assert.match(taskEvidenceView, /renderWebCard/)
  assert.match(conversationMessageView, /live-draft/)
  assert.match(conversationMessageView, /尚未定稿；Harness 提交后会由正式任务记录替换/)
  ...
  assert.match(conversationMessageView, /复制回答/)
  assert.match(conversationMessageView, /复制代码/)
...
test('failed model requests show human recovery and keep raw runtime context behind a second disclosure', () => {
  ...
  assert.match(taskEvidenceView, /item\.label.*item\.detail/)
})
...
test('completed tasks show a compact human result in the fixed sidebar with evidence in trace', () => {
  ...
  assert.match(taskEvidenceView, /state === 'passed' \? '通过' : state === 'failed' \? '未通过' : '未确认'/)
...
test('completed work has a Harness-backed visual receipt without introducing a second execution truth', () => {
  ...
  assert.match(taskEvidenceView, /renderOutcomeMap\(outcome\?\.map\)/)
  assert.match(shell, /setTaskView\('trace'\)/)
  assert.match(shell, /onEvidenceTarget: openTaskEvidenceTarget/)
  assert.match(main, /projectTaskOutcome\(thread\)/)
})

test('work receipts translate high-impact changes and rollback uncertainty for beginners', () => {
  assert.match(taskEvidenceView, /需要你留意的高影响改动/)
  assert.match(taskEvidenceView, /任务与改动归属/)
  assert.match(taskEvidenceView, /recoveryAssessment/)
  assert.match(taskEvidenceView, /任务开始前的本地 Git 基线/)
  assert.match(taskEvidenceView, /没有可用的 Git HEAD（与模型连接无关）/)
...
test('the composer exposes explicit model selection while Harness remains selection truth', () => {
  ...
  assert.match(html, /id="model-choice"[^>]+role="radiogroup"/)
  assert.match(html, /id="effort-choice"[^>]+role="radiogroup"/)
  assert.doesNotMatch(html, /<select id="(?:model|effort)-choice"/)
  assert.match(shell, /function selectChoice\(container, value\)/)
